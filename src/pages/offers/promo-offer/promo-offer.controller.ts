...
export class PromoOfferController {
  ...
  async getPromoOfferBySlug(
    ...
    return await this.promoOfferService.getPromoOfferBySlug(slug, select);
  }
  ...
  async getProductByOfferSlugAndProductSlug(
      @Param('slug') promoSlug: string,
      @Param('slug2') productSlug: string,
      @Query() select: string,
  ): Promise<ResponsePayload> {
    return await this.promoOfferService.getProductByOfferSlugAndProductSlug(promoSlug, productSlug);
  }
